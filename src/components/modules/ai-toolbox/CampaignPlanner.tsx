...
export function CampaignPlanner() {
  ...
  const handleSubmit = (data: CampaignPayload) => {
    setPayload(data);
    // Save to history
    const newItem: HistoryItem = { id: crypto.randomUUID(), payload: data, date: new Date().toISOString() };
          ...
          {history.map(item => (
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-medium text-foreground">{item.payload.brandName}</span>
