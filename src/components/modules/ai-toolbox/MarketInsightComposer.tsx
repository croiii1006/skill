...
import { ArrowUp, X, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    <div className="min-h-full flex flex-col items-center justify-center p-6 md:p-8 py-[80px]">
      <div className="w-full max-w-2xl animate-fade-in">
        ...
        <div className="text-center mb-10">
          <h1 className="text-2xl md:text-3xl font-normal text-foreground tracking-tight">品牌洞察分析报告</h1>
      ...
      {(() => {
        ...
        const previewCases = allMarketCases.slice(0, ITEMS_PER_PAGE);
        ...
        return allMarketCases.length > 0 ? (
          <div className="mt-10 w-full max-w-5xl">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
              {previewCases.map((card, i) => (
                <ShowcaseCard
                  ...
                  onClick={() => {
                    if (i === 0) {
                      window.open('https://haifeisianalysis.photog.art/', '_blank');
                    }
                  }}
                />
              ))}
            ...
            {/* View more / collapse */}
            {allMarketCases.length > ITEMS_PER_PAGE && (
              <button
                onClick={() => { setCasesExpanded(!casesExpanded); setCasesPage(0); }}
                className="flex items-center gap-1 mx-auto mt-5 text-xs text-muted-foreground hover:text-foreground transition-colors"
              >
                {casesExpanded ? '收起' : '查看更多'}
                <ChevronDown className={`w-3.5 h-3.5 transition-transform ${casesExpanded ? 'rotate-180' : ''}`} />
              </button>
            )}
          </div>
        ) : null;
